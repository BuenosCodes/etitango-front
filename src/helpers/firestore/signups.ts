...
export const getSignups = async (etiEventId: string) => {
    ...
    const data = querySnapshot.docs.map((doc) => ({id: doc.id, ...doc.data()})) as SignupFirestore[];
...
export const createSignup = async (etiEventId: string, userId: string, data: SignupCreate) => createDoc(SIGNUPS, {
    ...data,
    userId,
    etiEventId
});
