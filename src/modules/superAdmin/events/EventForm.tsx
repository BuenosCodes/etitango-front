...
import { assignEventAdmin } from '../../../helpers/firestore/users';
...
export default function EventForm() {
  ...
  const EventFormSchema = object({
    ...
    dateEnd: date()
      ...
      .when(
        'dateStart',
        (dateStart, schema) => dateStart && schema.min(dateStart, t('errors.dateEnd'))
      )
      .required(t('errors.alertText')),
    dateSignupOpen: date()
      .nullable()
    ...
    dateSignupEnd: date()
      ...
      .when(
        'dateSignupOpen',
        (dateSignupOpen, schema) =>
          dateSignupOpen && schema.min(dateSignupOpen, t('errors.dateSignupEnd'))
      )
      .required(t('errors.alertText')),
  ...
  return (
    <Translation ns={[SCOPES.MODULES.ETI]} useSuspense={false}>
      {(t) => (
        <>
          <WithAuthentication
          ...
          {loading ? (
            ...
            <Box sx={{ ...styles.mainContainer }}>
              <Box sx={{ ...styles.titleContainer }}>{t('placeholders.name')}</Box>
                                ...
                                <Grid container spacing={{ xs: 3, md: 1 }}>
                                  ...
                                  <Grid item xs={6} sm={6} md={3} lg={2}>
                                    <ETITimePicker
                                      ...
                                      onChange={(value: any) =>
                                        setFieldValue('timeSignupEnd', value)
                                      }
                                  ...
                                  >
                                    ...
                                    >
                                      {admins.length ? (
                                        ...
                                        <Typography
                                          typography={{ xs: 'label.mobile', md: 'body.regular.l' }}
                                          sx={{
                                            display: 'flex',
                                            alignItems: { xs: 'none', md: 'center' },
                                            ml: 1,
                                            color: { xs: 'greyScale.800', md: 'greyScale.500' }
                                          }}
                                        >
                                          {' '}
                                          {t('placeholders.organizers')}{' '}
                                        </Typography>
                                      )}
                                    </Box>
